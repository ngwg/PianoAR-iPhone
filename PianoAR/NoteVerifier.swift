...
final class NoteVerifier {
    ...
    func evaluate(pre: [Float], post: [Float], sampleRate: Float,
        ...
        for k in keys where k >= 0 && k < 88 {
            ...
            var bestSingle: (r: Float, snr: Float) = (0, 0)

            for n in 1...maxN {
                if n == 1 && f0 < 80 { continue }          // phone mics barely capture it
                let nf = Float(n)
                let fc = nf * f0 * sqrtf(1 + B * nf * nf)
                guard fc >= 60, fc <= maxHz else { if fc > maxHz { break } else { continue } }
                if sharesPartial(fc, withAnyOf: others, binHz: binHz) { continue }

                let fLo = nf * f0 * powf(2, -centsTol / 1200) * sqrtf(1 + 0.5 * B * nf * nf)
                let fHi = nf * f0 * powf(2,  centsTol / 1200) * sqrtf(1 + 2.0 * B * nf * nf)
                var lo = Int((fLo / binHz).rounded(.down))
                var hi = Int((fHi / binHz).rounded(.up))
                let c = Int((fc / binHz).rounded())
                lo = max(1, min(lo, c)); hi = min(after.count - 2, max(hi, c))
                ...
                let r   = 20 * log10f((aAfter + nfl) / (aBefore + nfl))
                let snr = 20 * log10f(aAfter / nfl)

                let w = (f0 + 52) / (nf * f0 + 320)
                sum  += w * simd_clamp(r, 0, 15)
                wSum += w
                usable += 1
                if r >= partialRiseDB && snr >= partialSNRDB { rising += 1 }
                if r > bestSingle.r, snr > 0 { bestSingle = (r, snr) }
            }

            let S = wSum > 0 ? sum / wSum : 0
            let status: NoteStatus
            if S >= presentDB && rising >= 2 {
                status = .present
            } else if f0 >= 1000 && usable <= 2 && bestSingle.r >= 9 && bestSingle.snr >= 12 {
                status = .present                           // treble: few partials exist
            } else if usable < 2 || S >= unsureDB {
                status = .unsure
            } else {
                status = .absent
            }
            out.append((k, simd_clamp(S / 8, 0, 1), status))
        }
        return out
    }
