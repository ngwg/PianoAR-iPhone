...
- **Note detection** — the sound decides. After every piano onset the microphone checks each expected note's own partials (chords: every note separately). A note counts when its own partials rose and no other key you actually hit explains them — **hand position is irrelevant**; your hands can be completely untracked and a note you played will still register. The hands are consulted for exactly one case: the upper note of an octave, where the sound genuinely cannot tell the two apart. One key stroke makes one onset, and the app can't credit the note it's waiting for to the sound of the note you just played. Clear wrong notes (a semitone or tone off) flash red.
- **It learns your piano** — no real piano is at A4 = 440.000, and all of them are stretch-tuned (bass flat, treble sharp). The app starts from the average curve and measures the real one per register as you play, then narrows its search accordingly. Nothing to set up, and it remembers between sessions. The debug panel shows it as `tune`; a `?` means that register hasn't been heard enough yet.
- **SETUP › RECORD** — saves the microphone plus every detection decision to **Files › PianoAR › Diagnostics** (a `.wav` and a `.jsonl`, up to 4 minutes). If detection ever misbehaves, record a minute of it and send both files: they say exactly what the app heard and why it decided what it did.
